...
concept IsMap = 
  ...
  requires {
    { declval<Type>().compute_keys() } -> IsSameType<typename Type::KeyListType>;
    { declval<Type>().compute_values() } -> IsSameType<typename Type::ValueListType>;
    { declval<Type>().contain_key(declval<typename Type::KeyType>()) } -> IsSameType<Bool>;
  };
